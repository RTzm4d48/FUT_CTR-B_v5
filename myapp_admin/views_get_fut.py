...
def log_verifying(log_position, id_admin):
    ...
    if (
        log_position == "treasury"
        or log_position == "secretary"
        or log_position == "direction"
    ):
        objs = (
            Admins.objects.filter(position=log_position)
...
def staff_treasury(request):
    ...
    )  # esto -> [::-1] es para invertir el orden del resultado
    num_futs_total = (
        fut.objects.exclude(stage__exact=3).filter(id_admin_turn=id_admin).count()
    )
...
def save_my_certifcate(tittle_, pdf_binary_, state_, fut_id_):
    my_objs = document(
        tittle=tittle_, pdf_binary=pdf_binary_, state=state_, fut_id_id=fut_id_
    )
...
def send_inssued(request):
    ...
    objs = (
        fut.objects.exclude(stage__exact=3)
        .filter(route=Position)
        .values("id", "order", "reason", "name", "dni", "code", "stage", "view")
    )
...
def view_send(request):
    if request.method == "GET":
        ...
        up_register = fut.objects.get(id=myid)
...
def postulated(request):
    ...
    print("mmmmmmmmPospuestosmmmmmmmm")
    print(login)
    print(Position)
    print(id_admin)
    print("-----")
    ...
    # (/AQUÍ)(ESTO_SE_REPITE)
    objs = fut.objects.filter(stage=1, route=Position).values(
        "order", "reason", "name", "dni", "code"
    )
    ...
    return render(
        request,
        "admin/staff/postulated.html",
        {"Object": list_data, "Data_log": Data_log, "Len_list_data": num_futs_total},
    )
...
def pending(request):
    ...
    return render(
        request, "admin/staff/pending.html", {"Object": list_data, "Data_log": Data_log}
    )
...
def send(request):
    ...
    objs_process = process.objects.filter(route=Data_log["send_position"]).values(
        "fut_id_id", "reception"
    )
    print("-------")
    print(objs_process)
    ...
    send_fut = (
        []
    )  # Lista oficial que tendra datos de 'FUT' y el 'TIME' de 'exit' de process
    for i in objs_process:
        diccionary = {}
        # FUT
        id_fut = i["fut_id_id"]
        objs = fut.objects.filter(id=id_fut).values(
            "id", "order", "reason", "name", "dni", "code"
        )
    ...
    return render(
        request, "admin/staff/send.html", {"Object": list_data, "Data_log": Data_log}
    )
...
def name_admin(position):
    if position == "treasury" or position == "secretary":
        ...
        objs = (
            Admins.objects.filter(position=next_position)
...
def admin_login(request):
    ...
    objs_email = (
        Admins.objects.filter(position=theposition, email=theemail).values("id").first()
    )
    objs_pass = (
        Admins.objects.filter(position=theposition, email=theemail, password=thepass)
        .values("id")
        .first()
    )
...
def next(request):
    ...
    objs = (
        ...
        .order_by("date")
        .values(
            "id", "order", "reason", "name", "dni", "code", "stage", "view", "date"
        )[::-1]
    )
